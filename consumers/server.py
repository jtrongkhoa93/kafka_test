...
def run_server():
    ...
    consumers = [
        # Consumer for weather
        KafkaConsumer(
            "org.chicago.cta.weather.v1",
            weather_model.process_message,
            offset_earliest=True,
        ),
        # Consumer for transformed Stations from Faust
        KafkaConsumer(
            # "org.chicago.cta.stations.table.v1",
            "org.chicago.cta.station.transformed-stations.v1"
            lines.process_message,
            offset_earliest=True,
            is_avro=False,
        ),
        # Consumer for arrival events
        KafkaConsumer(
            "^org.chicago.cta.station.arrivals.",
            lines.process_message,
            offset_earliest=True,
        ),
        # Consumer for turnstile summary from KSQL table
        KafkaConsumer(
            "TURNSTILE_SUMMARY",
            lines.process_message,
            offset_earliest=True,
